...
from ..mcp_logging import _log  # noqa: F401 — shared with mcp_server
...
class TaskContextManager:
    """Thread-safe registry of per-task working directories."""
    ...
    def register(self, task_id: str, cwd: str) -> None:
        with self._lock:
            self._contexts[task_id] = TaskContext(cwd=cwd)
    ...
    def cwd_for(self, task_id: str) -> str:
        ...
        return ctx.cwd if ctx is not None else ""


class VisiblePromptDeduplicator:
    """Suppresses repeated OS notifications for the same prompt fingerprint."""
    ...
    def notify(self, *, task_id: str, question: str, options: list[str], prompt_kind: str) -> None:
        ...
        normalized_opts = [o.strip() for o in options if o.strip()]
        fingerprint = f"{prompt_kind}|{normalized_q}|{'|'.join(normalized_opts)}"
        with self._lock:
            if self._seen.get(task_id) == fingerprint:
                return
            self._seen[task_id] = fingerprint
        self._on_new(task_id=task_id, question=normalized_q, options=normalized_opts, prompt_kind=prompt_kind)
