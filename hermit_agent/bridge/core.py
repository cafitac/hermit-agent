    ...
    while True:
        ...
        if msg.get("_source") in {"sse", "interactive_sse"}:
            ...
            if t == "done":
                ...
                token_totals = msg.get("token_totals") or {}
                if token_totals:
                    inp = token_totals.get("prompt_tokens", 0)
                    out = token_totals.get("completion_tokens", 0)
                    cached = token_totals.get("cached_tokens", 0)
                    reasoning = token_totals.get("reasoning_tokens", 0)
                    total = inp + out
                    cached_str = f" (+ {cached:,} cached)" if cached else ""
                    reasoning_str = f" (reasoning {reasoning:,})" if reasoning else ""
                    summary = f"Token usage: total={total:,} input={inp:,}{cached_str} output={out:,}{reasoning_str}"
                    _send({"type": "session_cost", "text": summary})
