...
from ..log_retention import DEFAULT_SESSION_MAX_KEEP
...
class SessionStore:
    ...
    def create_session(
        ...
        self._prune_mode_bucket(mode=mode, cwd=cwd, keep=DEFAULT_SESSION_MAX_KEEP, protected_session_id=session_id)
    ...
    def list_sessions(
        ...
        Uses mtime-first scanning to read only the most-recent meta.json files
        instead of all of them, reducing file I/O from O(total) to O(limit).
        """
        results: list[dict] = []
        has_filter = cwd is not None or parent_session_id is not None
        # Read more entries than limit when filters are active (they may reject some)
        scan_limit = limit * 4 if has_filter else limit

        # New layout: collect (mtime, sid_dir) pairs via stat, sort, then read meta.json
        root_path = Path(self.root)
        if root_path.exists():
            mode_dirs: list[Path]
            if mode is None:
                try:
                    mode_dirs = [e.path for e in os.scandir(root_path) if e.is_dir()]
                except OSError:
                    mode_dirs = []
            else:
                md = root_path / mode
                mode_dirs = [md] if md.is_dir() else []

            # Phase 1: stat all sid_dirs (cheap — no file reads)
            sid_entries: list[tuple[float, Path]] = []
            for md_path in mode_dirs:
                try:
                    for cwd_entry in os.scandir(md_path):
                        if not cwd_entry.is_dir():
                            continue
                        for sid_entry in os.scandir(cwd_entry.path):
                            if not sid_entry.is_dir():
                                continue
                            meta_path = Path(sid_entry.path) / 'meta.json'
                            if not meta_path.exists():
                                continue
                            try:
                                mtime = sid_entry.stat().st_mtime
                            except OSError:
                                mtime = 0.0
                            sid_entries.append((mtime, Path(sid_entry.path)))
                except OSError:
                    continue

            # Phase 2: read only the most-recent scan_limit entries
            sid_entries.sort(key=lambda t: t[0], reverse=True)
            for _, sid_dir in sid_entries[:scan_limit]:
    ...
    def _prune_mode_bucket(
        ...
        *,
        ...
        if keep <= 0:
            return
        bucket = Path(self.root) / mode / cwd_slug(cwd)
        if not bucket.exists():
            return

        candidates: list[tuple[float, Path]] = []
        for sid_dir in bucket.iterdir():
            if not sid_dir.is_dir():
                continue
            if protected_session_id is not None and sid_dir.name == protected_session_id:
                continue
            meta_path = sid_dir / "meta.json"
            if not meta_path.exists():
                continue
            ...
            if meta.get("status") == "active":
                continue
            candidates.append((_parse_updated_at(meta), sid_dir))

        excess = len(candidates) - keep
        if excess <= 0:
            return
        for _, sid_dir in sorted(candidates, key=lambda item: item[0])[:excess]:
            try:
                for child in sorted(sid_dir.rglob("*"), reverse=True):
