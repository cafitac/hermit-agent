...
| Wrong model picked | `HERMIT_MODEL` env or the `model` key in settings. Explicit routing: `gpt-*-codex` → Codex, `glm-*` → `providers["z.ai"]`, `name:tag` → ollama. If model is omitted, gateway auto-routes `codex -> z.ai -> local`. |
