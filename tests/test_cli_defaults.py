...
def test_main_dispatches_setup_codex(monkeypatch, capsys):
    from hermit_agent import __main__ as main_mod

    monkeypatch.setattr(main_mod.sys, "argv", ["hermit-agent", "setup-codex", "--cwd", "/tmp/demo"])
...
def test_main_dispatches_install_codex_alias(monkeypatch, capsys):
    from hermit_agent import __main__ as main_mod
